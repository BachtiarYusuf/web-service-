...
$link = mysqli_connect('localhost', 'id12777804_root', '123098', 'id12777804_akademik');
...
if (strcmp($data, 'data') ==0) {
 ...
 case 'GET':
    ...
    {
    $sql = "select * from pkm"; 
    echo "select * from pkm ";break;
    }
    else
    {
         $sql = "select * from pkm where nim='$id'";
         echo "select * from pkm where nim='$id'";break;
...
if ($method == 'POST') {
		...
		$querycek = "SELECT nim,nama,prodi FROM pkm WHERE nim ='$nimmhsw'";
		echo "query select ".$querycek;
		$result=mysqli_query($link,$querycek);
		
		if ( $rowcount == 0)
		{
		$query = "INSERT INTO pkm (
		nim,
		nama,
		prodi)
		VALUES (				
